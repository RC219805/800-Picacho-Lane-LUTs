...
def seed_all(seed: int) -> Generator:
    ...
    return Generator(device="cuda" if torch.cuda.is_available() else "cpu").manual_seed(
        seed
    )
...
def apply_material_response_finishing(
    ...
    from scipy.ndimage import (  # Lazy import to avoid hard dependency at module import time
    ...
    if (
        ...
        pool_blend = (
            pool_texture_strength
            * pool_mask[..., None]
            * np.clip(depth_falloff[..., None], 0.25, 1.0)
        )
    ...
    if floor_plank_contrast > 0:
        ...
        rgb = np.clip(
            rgb + floor_plank_contrast * plank_detail * floor_mask[..., None], 0.0, 1.0
        )
        ...
        rgb = np.clip(
            rgb + 0.12 * floor_plank_contrast * grain[..., None] * (warm_wood - rgb),
            0.0,
            1.0,
        )
    ...
    if window_reflection > 0:
        ...
        rgb = np.clip(
            rgb + window_reflection * reflection[..., None] * (reflection_color - rgb),
            0.0,
            1.0,
        )
    ...
    if (
        sky_environment_path
        and sky_environment_strength > 0
        and exterior_mask is not None
        and depth_falloff is not None
    ):
        ...
        sky_weight = (
            ...
            * exterior_mask
            * np.clip(depth_falloff**0.85, 0.0, 1.0)
        )
        sky_mix = 0.6 * sky_texture + 0.4 * rgb
        rgb = np.clip(
            rgb * (1.0 - sky_weight[..., None]) + sky_mix * sky_weight[..., None],
            0.0,
            1.0,
        )
    ...
    if bedding_relief > 0:
        ...
        rgb = np.clip(
            rgb - bedding_relief * 0.35 * bedding_mask[..., None] * shading[..., None],
            0.0,
            1.0,
        )
    ...
    if painting_integration > 0:
        ...
        rgb = np.clip(
            rgb * (1.0 - painting_integration * 0.12 * canvas_shadow[..., None]),
            0.0,
            1.0,
        )
...
def adjust_contrast_saturation(
    rgb: np.ndarray, contrast: float = 1.08, saturation: float = 1.05
) -> np.ndarray:
...
def overlay_logo_caption(
    img: Image.Image, logo_path: Optional[str], text: Optional[str], margin: int = 36
) -> Image.Image:
    canvas = img.copy()
    ...
    if text:
        try:
            ...
            font = ImageFont.truetype("arial.ttf", size=max(22, H // 40))  # type: ignore[assignment]
        except Exception:
    ...
    if logo_path and Path(logo_path).exists():
        ...
        logo = logo.resize(
            (int(logo.width * scale), target_h), Image.Resampling.LANCZOS
        )
        ...
        (
            ...
            if canvas.mode == "RGBA"
            else canvas.paste(logo, (lx, ly), logo)
        )
...
class LuxuryRenderPipeline:
    def __init__(
        ...
        self.cn_canny = ControlNetModel.from_pretrained(
            model_ids.controlnet_canny, torch_dtype=self.dtype
        )
        self.cn_depth = (
        ...
        if use_realesrgan and not _HAS_REALESRGAN:
            print(
                "[Warn] Real-ESRGAN requested but not installed. Run: pip install realesrgan basicsr"
            )
    ...
    def enhance(
        ...
        if finish.bloom:
            rgb = add_bloom(
                rgb, finish.bloom_threshold, finish.bloom_radius, finish.bloom_intensity
            )
        if finish.vignette:
...
def main(
    ...
    neg: str = typer.Option(
        "lowres, bad geometry, blurry, text, watermark, logo", help="Negative prompt"
    ),
    # Render config
    ...
    strength: float = typer.Option(
        0.5, help="How far to move from init (0.3–0.6 preserves structure)"
    ),
    ...
    material_response: bool = typer.Option(
        False, help="Enable texture + atmosphere finishing enhancements"
    ),
    texture_boost: float = typer.Option(
        0.25, help="Material response: detail boost strength"
    ),
    ambient_occlusion: float = typer.Option(
        0.12, help="Material response: contact shadow intensity"
    ),
    highlight_warmth: float = typer.Option(
        0.08, help="Material response: warm highlight mix"
    ),
    haze_strength: float = typer.Option(
        0.06, help="Material response: volumetric haze blend"
    ),
    haze_tint: str = typer.Option(
        "0.82,0.88,0.96", help="Material response: haze tint (r,g,b in 0-1)"
    ),
    floor_plank_contrast: float = typer.Option(
        0.12, help="Material response: enhance floor plank definition"
    ),
    floor_specular: float = typer.Option(
        0.18, help="Material response: specular streak intensity on flooring"
    ),
    floor_contact_shadow: float = typer.Option(
        0.05, help="Material response: floor transition contact shadow"
    ),
    floor_texture: Optional[str] = typer.Option(
        None, help="Material response: floor texture image path"
    ),
    ...
    floor_texture_scale: float = typer.Option(
        1.0, min=0.1, help="Material response: floor texture scale multiplier"
    ),
    textile_contrast: float = typer.Option(
        0.18, help="Material response: linen/fabric separation"
    ),
    leather_sheen: float = typer.Option(
        0.16, help="Material response: leather sheen blend"
    ),
    fireplace_glow: float = typer.Option(
        0.18, help="Material response: fireplace spill intensity"
    ),
    fireplace_glow_radius: float = typer.Option(
        45.0, help="Material response: fireplace glow falloff radius"
    ),
    window_reflection: float = typer.Option(
        0.12, help="Material response: window reflection spill on flooring"
    ),
    bedding_relief: float = typer.Option(
        0.16, help="Material response: bedding wrinkle relief and occlusion"
    ),
    wall_texture_path: Optional[str] = typer.Option(
        None, help="Material response: wall texture image path"
    ),
    ...
    wall_texture_scale: float = typer.Option(
        1.0, min=0.1, help="Material response: wall texture scale multiplier"
    ),
    wall_texture: float = typer.Option(
        0.1, help="Material response: wall microtexture strength"
    ),
    painting_integration: float = typer.Option(
        0.1, help="Material response: wall art integration and rim light"
    ),
    window_light_wrap: float = typer.Option(
        0.14, help="Material response: window light wrap onto interior surfaces"
    ),
    pool_texture_path: Optional[str] = typer.Option(
        None, help="Material response: pool tile texture image path"
    ),
    ...
    pool_texture_scale: float = typer.Option(
        1.0, min=0.1, help="Material response: pool texture scale multiplier"
    ),
    exterior_atmosphere: float = typer.Option(
        0.12, help="Material response: exterior haze and atmospheric blend"
    ),
    sky_environment_path: Optional[str] = typer.Option(
        None, help="Material response: sky environment plate (image path)"
    ),
    ...
    sky_environment_scale: float = typer.Option(
        1.0, min=0.1, help="Material response: sky environment horizontal scale"
    ),
    # Branding
    logo: Optional[str] = typer.Option(
        None, help="Path to PNG/SVG logo (PNG w/ alpha recommended)"
    ),
    brand_text: Optional[str] = typer.Option(
        None, help="Caption, e.g. 'The Veridian | Penthouse 21B'"
    ),
    # Export/resolution
    ...
    use_realesrgan: bool = typer.Option(
        False, help="Use Real-ESRGAN (requires weights, GPU recommended)"
    ),
):
    out_dir = Path(out)
