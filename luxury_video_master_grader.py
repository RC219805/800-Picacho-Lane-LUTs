...
def summarize_probe(data: Dict[str, object]) -> str:
    ...
    if video:
        ...
        fps = describe_frame_rates(
            cast(Optional[str], video.get("avg_frame_rate")), cast(Optional[str], video.get("r_frame_rate"))
        )
        ...
        color_primaries = normalise_color_tag(cast(Optional[str], video.get("color_primaries")))
        color_trc = normalise_color_tag(cast(Optional[str], video.get("color_trc")))
...
def plan_tone_mapping(
    ...
    transfer = (cast(str, video.get("color_trc")) or "").lower()
    primaries = (cast(str, video.get("color_primaries")) or "").lower()
...
def assess_frame_rate(
    ...
    streams = cast(List[Dict[str, object]], probe.get("streams", []))
    video: Optional[Dict[str, object]] = next((s for s in streams if s.get("codec_type") == "video"), None)
    if not video:
    ...
    avg_fraction = parse_ffprobe_fraction(cast(Optional[str], video.get("avg_frame_rate")))
    real_fraction = parse_ffprobe_fraction(cast(Optional[str], video.get("r_frame_rate")))
    ...
    if vfr:
        # Type narrowing: vfr is only True when both avg_fraction and real_fraction exist
        assert avg_fraction is not None and real_fraction is not None
        return FrameRatePlan(
...
def build_filter_graph(config: Dict[str, object]) -> Tuple[str, str]:
    ...
    eq_parts: List[str] = []
    contrast = float(cast(Any, config.get("contrast", 1.0)))
    saturation = float(cast(Any, config.get("saturation", 1.0)))
    gamma = float(cast(Any, config.get("gamma", 1.0)))
    brightness = float(cast(Any, config.get("brightness", 0.0)))
    ...
    warmth = float(cast(Any, config.get("warmth", 0.0)))
    cool = float(cast(Any, config.get("cool", 0.0)))
    ...
    lut_strength = float(cast(Any, config.get("lut_strength", 1.0)))
    ...
    grain = float(cast(Any, config.get("grain", 0.0)))
    if grain > 0.0:
        new_label = next_label()
    ...
    halation_intensity = float(cast(Any, config.get("halation_intensity", 0.0)))
    if halation_intensity > 0.0:
        intensity = clamp(halation_intensity, 0.0, 1.0)
        radius = clamp(float(cast(Any, config.get("halation_radius", 18.0))), 0.0, 128.0)
        radius = max(radius, 1.0)
        threshold = clamp(float(cast(Any, config.get("halation_threshold", 0.6))), 0.0, 1.0)
...
def determine_color_metadata(
    ...
    if args.color_from_source:
        video = extract_video_stream(probe)
        if video:
            primaries = normalise_color_tag(cast(Optional[str], video.get("color_primaries")))
            transfer = normalise_color_tag(cast(Optional[str], video.get("color_trc")))
...
def main(argv: Optional[Iterable[str]] = None) -> int:
    ...
    try:
        ...
        halation_radius = float(cast(Any, config.get("halation_radius", 0.0)))
        ...
        threshold_value = float(cast(Any, config.get("halation_threshold", 0.6)))
