# file: picacho_lane_luts/presence_cli_v1_3.py
...
def _sobel_magnitude(gray: np.ndarray) -> np.ndarray:
    ...
    d = p[1:-1, :-2]; e = p[1:-1, 1:-1]; f = p[1:-1, 2:]
    g = p[2:, :-2];  h = p[2:, 1:-1];  i = p[2:, 2:]
    gx = a*kx[0,0]+b*kx[0,1]+c*kx[0,2]+d*kx[1,0]+e*kx[1,1]+f*kx[1,2]+g*kx[2,0]+h*kx[2,1]+i*kx[2,2]
    gy = a*ky[0,0]+b*ky[0,1]+c*ky[0,2]+d*ky[1,0]+e*ky[1,1]+f*ky[1,2]+g*ky[2,0]+h*ky[2,1]+i*ky[2,2]
