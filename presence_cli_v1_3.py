...
def measure_image(path: str, aspect: str="4:5") -> Dict[str,float]:
    ...
    prom = (row_energy.max() - np.median(row_energy)) / (np.std(row_energy)+1e-6)
    alpha = max(0.3, min(0.85, 0.45 + 0.08*prom))  # 0.30..0.85
    ...
    if left_edge is None: left_edge = int(0.14*w)
    if right_edge is None: right_edge = int(0.86*w)
