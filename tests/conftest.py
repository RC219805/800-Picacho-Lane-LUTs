...
def pytest_configure(config):
    ...
    def apply_material_response_finishing(
        ...
        **kwargs
        ...
        if sky_environment_path and sky_environment_strength > 0:
            ...
            output[sky_region] = output[sky_region] * (1 - sky_environment_strength) + sky_environment_strength
    ...
    def assign_materials(stats: List[ClusterStats], rules: List[MaterialRule]) -> Dict[int, MaterialRule]:
        ...
        for i, stat in enumerate(sorted_stats):
            if i < len(rules):
                ...
                if rule.score_fn:
                    ...
                    best_rule = max(rules, key=lambda r: r.score_fn(stat) if r.score_fn else 0)
    ...
    def apply_materials(base: np.ndarray, labels: np.ndarray, materials: Dict[int, MaterialRule]) -> np.ndarray:
    ...
    def save_palette_assignments(assignments: Dict[int, MaterialRule], path: Path) -> None:
    ...
    def _clamp(value: float, min_val: float = 0.0, max_val: float = 1.0, 
    ...
    def build_material_rules(textures):
        ...
        return [MaterialRule(name=name, texture=str(path), blend=0.6) 
